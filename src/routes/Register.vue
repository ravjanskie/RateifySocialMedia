<template>
  <section class="">
    ...
    >
      ...
      <div
        class="w-full bg-slate-100 rounded-lg shadow dark:border md:mt-0 sm:max-w-md xl:p-0 dark:bg-gray-800 dark:border-gray-700"
      >
...
let lastname = ref("");
let username = ref("");
let avatar_url = ref("default-profile-pic.png");
let email = ref("");
let password = ref("");
...
async function create() {
  ...
  if (!firstname.value) {
    ...
    Swal.fire({
      icon: "error",
      title: "First name must be filled out",
  ...
  if (!lastname.value) {
    ...
    Swal.fire({
      icon: "error",
      title: "Last name must be filled out",
      text: "Please enter a value",
    });
    showLoader.value = false;
    return;
  ...
  try {
    ...
    //   ]);
      ...
      Swal.fire({
        icon: "success",
        title: "Email Verification is sent to your Email account",
